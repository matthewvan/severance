...
<?php } if ($this->_rootref['S_FORUM_RULES']) {  ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'] : ''; echo ((isset($this->_rootref['L_FORUM_RULES'])) ? $this->_rootref['L_FORUM_RULES'] : ((isset($user->lang['FORUM_RULES'])) ? $user->lang['FORUM_RULES'] : '{ FORUM_RULES }')); echo (isset($this->_tpldata['DEFINE']['.']['END_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['END_TCORNERS'] : ''; ?>

	<div class="rules"><p style="margin-bottom: 0; padding: 6px;">
...
<?php if ($this->_rootref['S_HAS_POLL']) {  ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'] : ''; echo (isset($this->_rootref['POLL_QUESTION'])) ? $this->_rootref['POLL_QUESTION'] : ''; echo (isset($this->_tpldata['DEFINE']['.']['END_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['END_TCORNERS'] : ''; ?>

	<div class="bg4">
		<div class="content">
			<p class="author" style="color: #FFFFFF;"><?php echo ((isset($this->_rootref['L_POLL_LENGTH'])) ? $this->_rootref['L_POLL_LENGTH'] : ((isset($user->lang['POLL_LENGTH'])) ? $user->lang['POLL_LENGTH'] : '{ POLL_LENGTH }')); if ($this->_rootref['S_CAN_VOTE'] && $this->_rootref['L_POLL_LENGTH']) {  ?><br /><?php } if ($this->_rootref['S_CAN_VOTE']) {  echo ((isset($this->_rootref['L_MAX_VOTES'])) ? $this->_rootref['L_MAX_VOTES'] : ((isset($user->lang['MAX_VOTES'])) ? $user->lang['MAX_VOTES'] : '{ MAX_VOTES }')); } ?></p>
	...
	<hr />
		<br />
<?php } $_postrow_count = (isset($this->_tpldata['postrow'])) ? sizeof($this->_tpldata['postrow']) : 0;if ($_postrow_count) {for ($_postrow_i = 0; $_postrow_i < $_postrow_count; ++$_postrow_i){$_postrow_val = &$this->_tpldata['postrow'][$_postrow_i]; if ($_postrow_val['S_FIRST_UNREAD']) {  ?><a id="unread"></a><?php } if ($_postrow_val['S_FIRST_ROW']) {  echo (isset($this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'] : ''; echo $_postrow_val['POST_SUBJECT']; echo (isset($this->_tpldata['DEFINE']['.']['END_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['END_TCORNERS'] : ''; } else { ?><div class="breadcrumbs"><?php echo $_postrow_val['POST_SUBJECT']; ?></div><?php } ?>

	<div id="p<?php echo $_postrow_val['POST_ID']; ?>" class="post bg4<?php if ($_postrow_val['S_UNREAD_POST']) {  ?> unreadpost<?php } if ($_postrow_val['S_POST_REPORTED']) {  ?> reported<?php } if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?> online<?php } ?>">
		...
		<div class="postbody">
			...
			<?php } } if ($_postrow_val['POST_ICON_IMG']) {  ?><img src="<?php echo (isset($this->_rootref['T_ICONS_PATH'])) ? $this->_rootref['T_ICONS_PATH'] : ''; echo $_postrow_val['POST_ICON_IMG']; ?>" width="<?php echo $_postrow_val['POST_ICON_IMG_WIDTH']; ?>" height="<?php echo $_postrow_val['POST_ICON_IMG_HEIGHT']; ?>" alt="" /> <?php } ?>
		...
		<?php if (! $_postrow_val['S_IGNORE_POST']) {  ?>
			...
			<dt>
			<strong><?php if (! $_postrow_val['U_POST_AUTHOR']) {  echo $_postrow_val['POST_AUTHOR_FULL']; } else { echo $_postrow_val['POST_AUTHOR_FULL']; } ?>&nbsp;<img src="<?php echo (isset($this->_rootref['T_THEME_PATH'])) ? $this->_rootref['T_THEME_PATH'] : ''; ?>/images/icon_user_<?php if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  ?>online<?php } else { ?>offline<?php } ?>.gif" alt="<?php if ($_postrow_val['S_ONLINE'] && ! $_postrow_val['S_IGNORE_POST']) {  echo ((isset($this->_rootref['L_ONLINE'])) ? $this->_rootref['L_ONLINE'] : ((isset($user->lang['ONLINE'])) ? $user->lang['ONLINE'] : '{ ONLINE }')); } else { echo ((isset($this->_rootref['L_OFFLINE'])) ? $this->_rootref['L_OFFLINE'] : ((isset($user->lang['OFFLINE'])) ? $user->lang['OFFLINE'] : '{ OFFLINE }')); } ?>" /></strong><br /><br />
			
				<?php if ($_postrow_val['POSTER_AVATAR']) {  ?>

					<br /><?php if ($_postrow_val['U_POST_AUTHOR']) {  ?><a href="<?php echo $_postrow_val['U_POST_AUTHOR']; ?>"><?php echo $_postrow_val['POSTER_AVATAR']; ?></a><?php } else { echo $_postrow_val['POSTER_AVATAR']; } ?><br />
				<?php } ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['BCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BCORNERS'] : ''; ?>
		...
		<hr class="divider" />
<?php }} if ($this->_rootref['S_QUICK_REPLY']) {  ?>

		<hr />
	<?php $this->_tpl_include('quickreply_editor.html'); } if ($this->_rootref['S_NUM_POSTS'] > (1) || $this->_rootref['PREVIOUS_PAGE']) {  ?>
...
<?php $this->_tpl_include('jumpbox.html'); if ($this->_rootref['S_DISPLAY_ONLINE_LIST']) {  ?>
	...
	<?php echo (isset($this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BEGIN_TCORNERS'] : ''; if ($this->_rootref['U_VIEWONLINE']) {  ?><a href="<?php echo (isset($this->_rootref['U_VIEWONLINE'])) ? $this->_rootref['U_VIEWONLINE'] : ''; ?>"><?php echo ((isset($this->_rootref['L_WHO_IS_ONLINE'])) ? $this->_rootref['L_WHO_IS_ONLINE'] : ((isset($user->lang['WHO_IS_ONLINE'])) ? $user->lang['WHO_IS_ONLINE'] : '{ WHO_IS_ONLINE }')); ?></a><?php } else { echo ((isset($this->_rootref['L_WHO_IS_ONLINE'])) ? $this->_rootref['L_WHO_IS_ONLINE'] : ((isset($user->lang['WHO_IS_ONLINE'])) ? $user->lang['WHO_IS_ONLINE'] : '{ WHO_IS_ONLINE }')); } echo (isset($this->_tpldata['DEFINE']['.']['END_TCORNERS'])) ? $this->_tpldata['DEFINE']['.']['END_TCORNERS'] : ''; ?>
		...
		<p style="background: #202020; margin-bottom: 0; padding: 6px;"><?php echo (isset($this->_rootref['LOGGED_IN_USER_LIST'])) ? $this->_rootref['LOGGED_IN_USER_LIST'] : ''; ?></p>
	<?php echo (isset($this->_tpldata['DEFINE']['.']['BCORNERS'])) ? $this->_tpldata['DEFINE']['.']['BCORNERS'] : ''; ?>
